...
const UserResults: React.FC = () => {
  ...
  const handleViewResult = (predictionWithResult: PredictionWithResult) => {
    if (predictionWithResult.result) {
      setSelectedResult(predictionWithResult.result);
  ...
  const renderWinningNumbers = (result: Result) => {
    if (result.winningNumbers && result.winningNumbers.whiteBalls) {
      // Double selection lottery
      return (
        <div>
          <div className="mb-3">
            <h6 className="fw-bold mb-2">White Balls:</h6>
            <div className="d-flex flex-wrap gap-2">
              {result.winningNumbers.whiteBalls.map((num, idx) => (
                <span key={idx} className="badge bg-primary fs-5 px-3 py-2">{num}</span>
              ))}
            </div>
          </div>
          {result.winningNumbers.redBalls && result.winningNumbers.redBalls.length > 0 && (
            <div>
              ...
              <div className="d-flex flex-wrap gap-2">
                {result.winningNumbers.redBalls.map((num, idx) => (
    ...
    } else if (result.winningNumbersSingle && result.winningNumbersSingle.length > 0) {
      // Single selection lottery
      return (
        <div>
          ...
          {selectedResult ? (
            <div>
              <div className="text-center mb-4">
                <h4 className="fw-bold">{selectedResult.lotteryType.charAt(0).toUpperCase() + selectedResult.lotteryType.slice(1)}</h4>
                <p className="text-muted">
                  <i className="bi bi-calendar me-2"></i>
                  Draw Date: {new Date(selectedResult.drawDate).toLocaleDateString()}
                </p>
              </div>
